@component('layouts.admin.master')
    ...
    @section('content')
        <div class="title-block">
            <h3 class="title"><i class="fa fa-edit"></i> Edit Category <span class="sparkline bar"
                                                                            data-type="bar"></span></h3>
        </div>
        <form class="form-horizontal" role="form" method="POST" action="{{ route('admin.categories.update', [$category]) }}">
            {{ method_field('PUT') }}
            {{ csrf_field() }}
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="name" class="form-control boxed" id="name"
                               placeholder="Enter Category Name" value="{{ old('name', $category->name) }}" autofocus>
                        @if ($errors->has('name'))
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea name="details" rows="5" class="form-control boxed" id="details"
                                  placeholder="Enter Category Details">{{ old('details', $category->details) }}</textarea>
