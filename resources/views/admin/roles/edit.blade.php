@component('layouts.admin.master')
    ...
    @section('content')
        ...
        <form class="form-horizontal" role="form" method="POST" action="{{ route('admin.roles.update', [$role]) }}">
            {{ method_field('put') }}
            {{ csrf_field() }}
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="title" class="form-control boxed" id="title"
                               placeholder="Enter Role Title" value="{{ old('title', $role->name) }}">
                    </div>
                </div><!-- /.form-group -->
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                            <textarea name="details" rows="5" class="form-control boxed" id="details"
                                      placeholder="Enter Role Details">{{ old('details', $role->details) }}</textarea>
                    </div>
                </div><!-- /.form-group -->
                <div class="form-group row">
