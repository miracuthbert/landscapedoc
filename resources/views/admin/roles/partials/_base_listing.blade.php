<li class="item">
    <div class="item-row">
        ...
        <div class="item-col fixed pull-left item-col-title">
            ...
            <div>
                <a href="{{ route('admin.roles.edit', [$role]) }}" class="">
                    <h4 class="item-title"> {{ $role->name }}</h4>
                </a>
            </div>
        </div>
        ...
        <div class="item-col item-col-stats no-overflow">
            <div class="item-heading">Users</div>
            <div class="no-overflow">
                <div class="item-stats">
                    <span class="badge badge-pill badge-primary">{{ $role->users->count() }}</span>
                    <a href="{{ route('admin.roles.show', [$role]) }}" title="Add new {{ $role->name }}"><i class="fa fa-user-plus"></i></a>
                </div>
            </div>
        </div>
                ...
                <div class="item-actions-block">
                    <ul class="item-actions-list">
                        <li>
                            ...
                            @include('layouts.admin.partials._confirm_modal', ['modal_id' => "confirm-modal-{$role->id}", 'title' => "Delete Confirmation", 'message' => "Are you sure you want to delete {$role->name}?", 'action' => "role-destroy-{$role->id}-form"])
                        </li>
                        <li>
